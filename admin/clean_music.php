...
$uploadPath = '../uploads/';
// 音频后缀对应mime：mp3,wav,m4a,ogg,flac
$audioMimeExts = ['mp3','webm','wav','m4a','ogg','flac'];
$msg = '';
$logLines = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['del_file'])) {
        ...
        if (is_file($fullPath)) {
            unlink($fullPath);
            $msg = "成功删除音频文件：" . basename($file);
            $logLines[] = "[$now] 删除音频：" . basename($file);
        } else {
            $msg = "文件不存在，删除失败";
    ...
    if (!empty($_POST['batch_files']) && is_array($_POST['batch_files'])) {
        ...
        $msg = "批量选中删除完成，共删除 {$batchDelCount} 个音频文件";
        $logLines[] = "[$now] 多选批量删除音频，删除总数：{$batchDelCount}";
    }
    // 一键清空所有音频
    if (isset($_POST['clear_all'])) {
        ...
        $msg = "批量清理完成，共删除 {$delCount} 个音频文件";
        $logLines[] = "[$now] 批量清理上传音频，删除总数：{$delCount}";
    }
}
// 统计音频信息
$audioList = [];
...
$logLines[] = "[$now] 进入音频清理面板";
$logLines[] = "[$now] 上传目录：{$uploadPath}";
$logLines[] = "[$now] 当前筛选后音频总数：{$audioCount} 个，占用空间：{$sizeText}";
$sortText = [
    'time' => '修改时间(默认)',
    'size' => '文件大小',
    'name' => '文件名'
...
<div class="wrap">
    <div class="sidebar">
        ...
        <ul class="side-menu">
            ...
            <li class="side-item"><a href="clean_music.php" class="side-link active"><i class="fas fa-music"></i> 音乐音频清理</a></li>
            <li class="side-item"><a href="../chat.php" class="side-link"><i class="fas fa-comments"></i> 返回聊天室</a></li>
        </ul>
    </div>
    <div class="main">
        <div class="page-head">
            <div class="page-title">上传音乐音频清理</div>
        ...
        <div class="card-box">
            <div class="card">
                <div class="card-body">
                    <div>
                        <div class="card-title">筛选后音频总数</div>
                        <div class="card-num"><?php echo $audioCount ?></div>
                    </div>
        ...
        <div class="opt-bar">
            <form method="post" onsubmit="return confirm('确认删除全部音频？此操作不可恢复！')">
                <button type="submit" name="clear_all" class="btn btn-danger"><i class="fas fa-trash-alt"></i> 一键清空所有音频</button>
            </form>
            <form id="batchForm" method="post" onsubmit="return confirm('确定删除选中的音频？不可恢复！')">
        ...
        <div class="table-box">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <form method="post" style="display:inline" onsubmit="return confirm('确定删除该音频？')">

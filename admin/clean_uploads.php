...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['del_file'])) {
        ...
        if (is_file($fullPath)) {
            unlink($fullPath);
            $msg = "成功删除文件：" . basename($file);
            $logLines[] = "[$now] 删除图片：" . basename($file);
        } else {
            $msg = "文件不存在，删除失败";
    ...
    if (!empty($_POST['batch_files']) && is_array($_POST['batch_files'])) {
        ...
        $msg = "批量选中删除完成，共删除 {$batchDelCount} 张图片";
        $logLines[] = "[$now] 多选批量删除图片，删除总数：{$batchDelCount}";
    }
    // 一键清空所有图片
    if (isset($_POST['clear_all'])) {
        ...
        $msg = "批量清理完成，共删除 {$delCount} 张图片";
        $logLines[] = "[$now] 批量清理上传图片，删除总数：{$delCount}";
    }
}
// 统计图片信息
$imgList = [];
                ...
                if ($quickFilter !== '') {
                    ...
                    switch ($quickFilter) {
                        case '7':
                            ...
                            break;
                        case '14':
                            $matchDate = $diffDay <= 14;
                            break;
                        case '30':
                            $matchDate = $diffDay <= 30;
                            break;
                        case 'over30':
                            $matchDate = $diffDay > 30;
                            break;
...
switch ($sortType) {
    case 'size':
        usort($imgList, function($a, $b) {
            return $b['size'] - $a['size']; // 大小从大到小
        });
        break;
    case 'name':
        usort($imgList, function($a, $b) {
            return strcmp($a['name'], $b['name']); // 文件名字母升序
        });
        break;
    default: // time 默认：修改时间新的在前
        usort($imgList, function($a, $b) {
...
$logLines[] = "[$now] 进入图片清理面板";
$logLines[] = "[$now] 上传目录：{$uploadPath}";
$logLines[] = "[$now] 当前筛选后图片总数：{$imgCount} 张，占用空间：{$sizeText}";
$sortText = [
    'time' => '修改时间(默认)',
    'size' => '文件大小',
    'name' => '文件名'
...
if ($quickFilter !== '') {
    $quickText = [
        '7'=>'近7天','14'=>'近14天','30'=>'近30天','over30'=>'30天以前'
    ][$quickFilter];
...
<div class="wrap">
    ...
    <div class="sidebar">
        ...
        <ul class="side-menu">
            ...
            <li class="side-item"><a href="clean_uploads.php" class="side-link active"><i class="fas fa-image"></i> 图片清理</a></li>
            <li class="side-item"><a href="clean_videos.php" class="side-link"><i class="fas fa-video"></i> 视频清理</a></li>
    ...
    <div class="main">
        <div class="page-head">
            <div class="page-title">上传图片清理</div>
        ...
        <div class="opt-bar">
            <form id="filterForm" method="get" style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
                ...
                <span>日期筛选：</span>
                <input class="filter-input" type="date" name="start" value="<?php echo htmlspecialchars($startDate) ?>" placeholder="起始日期">
                <span>~</span>
                <input class="filter-input" type="date" name="end" value="<?php echo htmlspecialchars($endDate) ?>" placeholder="结束日期">
                ...
                <a href="?sort=<?php echo $sortType ?>" class="btn btn-outline">
        ...
        <div class="card-box">
            <div class="card">
                <div class="card-body">
                    <div>
                        <div class="card-title">筛选后图片总数</div>
                        <div class="card-num"><?php echo $imgCount ?></div>
                    </div>
        ...
        <!-- 操作+排序按钮 -->
        <div class="opt-bar">
            <form method="post" onsubmit="return confirm('确认删除全部图片？此操作不可恢复！')">
                <button type="submit" name="clear_all" class="btn btn-danger">
                    <i class="fas fa-trash-alt"></i> 一键清空所有图片
                </button>
            </form>
            <!-- 批量操作表单 -->
            <form id="batchForm" method="post" onsubmit="return confirm('确定删除选中的图片？不可恢复！')">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-check-square"></i> 删除选中项
                </button>
            </form>
            <!-- 全选、反选、取消选择按钮 -->
            ...
            <button type="button" class="btn btn-outline" onclick="reverseSelect()">
                <i class="fas fa-exchange-alt"></i> 反向选择
            </button>
            <!-- 取消选择按钮，默认隐藏 -->
            <button id="cancelSelectBtn" type="button" class="btn btn-outline" style="display:none;" onclick="clearSelect()">
        ...
        <div class="table-box">
            ...
            <table>
                ...
                <tr>
                    <td>
                        <input type="checkbox" form="batchForm" name="batch_files[]" class="item-check" value="<?php echo htmlspecialchars($item['name']) ?>">
                    </td>
                    <td><?php echo htmlspecialchars($item['name']) ?></td>
                    <td><?php echo formatSize($item['size']) ?></td>
                    <td><?php echo $item['show_time'] ?></td>
                    <td>
                        <form method="post" style="display:inline" onsubmit="return confirm('确定删除该图片？')">
...
function getCheckItems() {
    return document.querySelectorAll('.item-check');
}
// 更新取消选择按钮显隐
...
// 表头全选框联动
function toggleAll(ckAll) {
    const items = getCheckItems();
    items.forEach(item => item.checked = ckAll.checked);
    refreshCancelBtn();
}
// 一键全选
function selectAll() {
    const items = getCheckItems();
    const ckAll = document.getElementById('checkAll');
